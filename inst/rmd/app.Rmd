...
plot2rend <- function(plottab,seltab,row,plot_group,plot_plot,fig) {
    req(row)
    req(NROW(seltab)>0)
    req(NROW(plottab)>0)
...
rf_gen_sel_plot_tab <- reactive({
    validate(need(NROW(rv_state$out$tab$ms1_plot) > 0 ||
                  NROW(rv_state$out$tab$ms2_plot) > 0,
                  message = "No plots found. Did you generate them?"),
             need(NROW(rv_state$out$tab$comp) > 0,
                  message = "Data extraction needs to be performed first."))

    rows <- rf_summ_table_rows()
    mp1 <- rv_state$out$tab$ms1_plot
    mp2 <- rv_state$out$tab$ms2_plot
    comp <- rv_state$out$tab$comp
...
output$plot_ms1_eic <- renderPlot({
    ...
    ## req(NROW(tab)>0)
    ## req(NROW(ms1_plot)>0)
    ## sel_grp <- tab[row,.SD,.SDcols=c(plot_group,plot_plot)]
    ## fig_eic <- ms1_plot[sel_grp,on=c(plot_group,plot_plot),nomatch=NULL]$fig_eic[[1]]
    ## fig_eic
})
