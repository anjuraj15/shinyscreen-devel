...
Shinyscreen can summarise the prescreening results in a table (_Browser_ tab) as well as visualise the spectra (_viewer_ tab). The summary table can be manually updated and the changes can be saved before exporting the _CSV_ file. The visualised plots can be saved as _pdf_ output under _Figures_ in the project directory.
...
the criteria defined in the _Prescreening_ tab is done , the resulting _summary table_ is given below to manually update the quality of the retrieved spectra.

To update the sumary table,

* Click any row of MS1 table and the corresponding MS2 peaks are shown on the right side pane.
...
* For any given ID, the MS1 and MS2 fragments can be visualized by selecting the corresponding row in the table.

* The user can zoom-in by selection the region of interest using the cursor and double click to zoom-out.

* The user can select multiple rows to display different plots  and allows the user to save all the plots or save individual plot by clicking _save this plot_ tab.
...
shinyscreen_server <- shinyscreen:::mk_shinyscreen_server()
```
