...
```{r, echo=F} 
selectInput("sort1",label="Group the compound index first by", choices=ARRANGE_CHOICES, selected="quality")
```

```{r, echo=F} 
selectInput("sort2",label="then", choices=ARRANGE_CHOICES, selected="set")
```

```{r, echo=F} 
selectInput("sort3",label="then", choices=ARRANGE_CHOICES, selected="adduct")
```

```{r, echo=F} 
selectInput("sort4",label="and finally", choices=ARRANGE_CHOICES, selected="tag")
```
...
<div style="width: 75%; padding-left: 1.0em;">
### Viewer
</div> <!-- Big right panel. -->
