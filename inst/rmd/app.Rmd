...
```{r,echo=F}
verbatimTextOutput("plot_hover_out")
````
```{r, echo=F}
wellPanel(id = "tPanel",style = "overflow-y:scroll;max-height:80vh; width: 100%; background: white",
          column(6,
...
output$plot_hover_out <- renderText({
    pr <- "Pointer position: "
    paste0(pr,'(',
           format(input$plot_hover[[1]],digits=5),
           ',',
           format(input$plot_hover[[2]],digits=2,scientific=T),
           ')')})
```
